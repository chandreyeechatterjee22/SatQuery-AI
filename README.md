...
| All-India locations | "State / UT" and "District / City" dropdowns (Map Analysis sidebar and the Fetch panel) cover all 28 states + 8 UTs and 734 districts, with a "Major cities" group first. Picking a district flies the map there. Data: geoBoundaries (2021 districts), generated once into `backend/data/india_locations.json` (87 KB, no polygons) by `scripts/build_india_locations.py`; see [backend/data/README.md](backend/data/README.md) for source, licence and limits. Old names such as "Bengaluru Urban", "Mumbai", "Chennai", "Mangaluru" still resolve. |
